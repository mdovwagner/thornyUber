...
export function scoreCards(G, ctx) {
  ...
  player.validOfficials[officials.POSTALCARRIER] = false; // Check for carriages
  ...
  if (player.tableau.length >= 7) {
    ...
    if (G.bonuses["Distance7"].length > 0) {
      if (!player.bonuses["Distance7"]) player.bonuses["Distance7"] = []; // Add bonus to player's bonuses

      player.bonuses["Distance7"].push(G.bonuses["Distance7"][0]); // Remove bonus from pile
